...
# Подключение к БД
conn = orac.connect(cfg.user_db + '/' + cfg.pass_db + '@' + cfg.db)
...
colum = ['Объем ДД', 'Значение приоритета корректировки', 'Доля ГЭС', 'Доля ТЭС', 'Факт общий по ГТПП/ГТП экспорта, кВт', 'Номер пакета ДД']
